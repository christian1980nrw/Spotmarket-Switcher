...
-   Om du använder ett Linux-system som Ubuntu eller Debian:
    ...
    -   kommandona är`cd /path/to/save/ &&  curl -s -O "https://raw.githubusercontent.com/christian1980nrw/Spotmarket-Switcher/main/scripts/{controller.sh,sample.config.txt}" && mv sample.config.txt config.txt && chmod +x ./controller.sh`och att redigera`vi /path/to/save/config.txt`
...
Dessutom, om du är med**Tyskland**och intresserad av att byta till en dynamisk eltaxa kan du stödja projektet genom att anmäla dig med denna[Tibber (remisslänk)](https://invite.tibber.com/ojgfbx2e)eller genom att ange koden**Ajjfbkse**i din app. Både du och projektet får en**50 euro bonus för hårdvara**. Observera att en smart mätare eller en Pulse-IR krävs för en timtaxa (<https://tibber.com/de/store/produkt/pulse-ir>) .
Om du behöver en naturgastariff eller föredrar en klassisk eltaxa kan du fortfarande stödja projektet[Octopus Energy (remisslänk)](https://share.octopusenergy.de/glass-raven-58).
...
Användare från**Österrike**kan stötta oss genom[aWATTar Österrike (referenslänk)](https://www.awattar.at/services/offers/promotecustomers)remissåtgärd och gå in**Aqqhamqnif**som kod.
